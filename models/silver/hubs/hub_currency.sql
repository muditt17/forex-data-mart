...
with src as (
  select distinct from_currency as currency_code
  from {{ ref('transactions_raw') }}
  where from_currency is not null

  union
  select distinct to_currency as currency_code
  from {{ ref('transactions_raw') }}
  where to_currency is not null

  union
  -- cash leases (current only from snapshot)
  select distinct currency_code
  from {{ ref('cash_lease_snapshot') }}
  where dbt_valid_to is null and currency_code is not null
...
{% if is_incremental() %}
  where {{ hash_key(['currency_code']) }} not in (select currency_hk from {{ this }})
{% endif %}